...
**Status:** v0.4 — **P1b built (dogfood increment)** (`@casper/ai`): registry (definitions as data, seeded by casper-sales), run engine (runs → `ai_runs`, every model turn + tool call → `ai_run_steps`), model gateway (opus-4-8 + adaptive thinking, prompt-cached stance, per-run token/cost accounting), M1 7-tool set behind `runTool` (allowlist → policy → zod → `can()` against the **owner** per D-022 → execute → step log; `ai.tool_denied` on refusal); propose tools write only into the run's change set, run ends at `preview_ready` — commit stays human (D-008). SSE to the dock via casper-web's `/api/ai/run`. **Deviations (tracked for P1c):** loop is **in-process**, not a WDK workflow (D-019); plan object is a static placeholder — no `awaiting_plan_approval`/`clarifying` engine states (clarify is scripted in the dock pre-run); budgets declared but **not enforced** (max-iterations cap only; the `ai_budget_counters` table exists but nothing writes it); registry is in-memory config, no `assistants` table; `dataBlock()` exported but unused in the M1 loop; **no tests yet** — the structural-safety success criteria below are unverified by test. | **Layer:** AI | **Phases:** 1+ | **Depends on:** casper-auth, casper-records, casper-workflow, casper-changesets, casper-events, casper-platform | **Used by:** casper-sales (assistant definitions), casper-feedback, casper-web, casper-api (run executor) | **Aligned with:** master-plan v0.8 (D-004, D-007, D-008, D-009, D-016, D-017, D-019, D-022, D-023, D-024, D-025)
